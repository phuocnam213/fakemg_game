...
bool Game::init() {
    ...
    if (success == true) commonFunc::renderWindow("The Doge Game", SCREEN_WIDTH, SCREEN_HEIGHT);
...
int Game::FPSCounterHit() {  // Đổi kiểu trả về thành int
    float timeInSeconds = fpsTimer.getTicks() / 1000.0f;  // Chuyển đổi đúng kiểu float
    if (timeInSeconds <= 0) return 0;  // Tránh chia cho 0
    
    int avgFPS = countedFrames / timeInSeconds;  // Tính FPS
    if (avgFPS > 2000000) {
        avgFPS = 0;  // Giới hạn giá trị để tránh lỗi
    }
    return avgFPS;  // Trả về giá trị FPS
}
