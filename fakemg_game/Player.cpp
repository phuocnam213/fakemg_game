...
void Player::getHit(vector<Skeleton*> &skeletonList, Mix_Chunk* p_sfx[], SDL_Rect& camera) {
    ...
    int maxHitsPerSec = 4; 
    Uint32 hitCooldown = 1000 / maxHitsPerSec; 

    for (auto* skeleton : skeletonList) {
        if (skeleton && skeleton->getDistance() <= TILE_WIDTH * 1.5 && skeleton->isAttacking() &&
            getY() >= skeleton->getY() - TILE_WIDTH && getY() <= skeleton->getY() + TILE_WIDTH * 0.5) {
            
            if (currentTime - lastHitTime >= hitCooldown) {
                lastHitTime = currentTime; 
                if (++hitCount >= 100) dead = true;
                Mix_PlayChannel(-1, p_sfx[hitSFX], 0);
            }
        }
    }

    if (getY() + PLAYER_HEIGHT >= LEVEL_HEIGHT || getX() - camera.x <= 128) dead = true;
}
